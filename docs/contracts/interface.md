...
`0x83` remains the four-byte legacy diagnostic payload `battery_mv:u16 | max_temp_c:u8 | legacy_fault_mask:u8`; `battery_mv=0` means sensing is not fitted and must not be interpreted as undervoltage. Additive telemetry `0x84` is 34 bytes, all multibyte fields big-endian: `fault_mask:u32 | mode:u8 | reset_cause:u8 | imu_age:u16 | wheel_l_age:u16 | wheel_r_age:u16 | servo_age[4]:u16 | imu_errors:u16 | wheel_errors[2]:u16 | servo_errors[4]:u16`. Age `0xffff` means no valid frame has ever been received. Error counters are saturated totals of timeout, checksum, and protocol failures. Reset-cause bits are pin, power-on, software, independent-watchdog, window-watchdog, low-power, and brownout in bits 0 through 6. Decoders may ignore unknown additive telemetry types.
