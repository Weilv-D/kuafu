...
`rl/env/contract.py` is the normative executable contract at schema `v1.2.0`. This document is the readable form.
...
Version is `1`. A `0x03` HELLO payload contains the 16-character lowercase physical model hash and starts a new receive session. Partial DMA chunks are retained, CRC-invalid bytes are resynchronized, and replayed/out-of-order sequences are discarded. A heartbeat payload contains `mode:u8`, `vx:i16/1000`, `wz:i16/1000`, and `D0:i16` millimetres; an action payload contains six `i16/10000` normalized actions. STM32 accepts ACTIVE only after a proven-compatible link (an explicit HELLO, or — because the Pi does not repeat HELLO after a firmware reset — the first heartbeat whose every field passes validation) plus a fresh heartbeat. Field semantics are mode-scoped on the firmware side: `vx`/`wz` are honoured in ACTIVE only (STAND is a position hold, CLIMB drives `D0` only), `D0` is honoured in STAND/CLIMB/ACTIVE, and residuals are honoured in ACTIVE with a fresh action — senders must not rely on the firmware to reinterpret fields outside their mode, but the firmware also does not trust a sender to zero them.

Telemetry uses `0x81` for IMU payload `[roll,pitch,yaw,gyro_x,gyro_y,gyro_z]`, all `i16/1000`, and `0x82` for joints in wire order `[wheel_L, wheel_R, A_l, A_r, B_l, B_r]`, each group containing `(pos,vel,tau/current)`. Velocity and angular-rate fields use `×1000` and wheel torque uses `×10000`. The Pi runtime reorders hips to Actor order `[A_l,B_l,A_r,B_r]`, derives the 35-value frame, and rejects telemetry older than 100 ms. A stale-telemetry rejection never silences the sender: the policy node falls back to an explicit safe request (STAND, zero commands, zero residual) so the heartbeat contract survives the outage, and a telemetry-side sequence restart (STM32 reboot) resynchronizes after a bounded burst of sequence-gated rejections instead of blackholing the stream.

`0x83` remains the four-byte legacy diagnostic payload `battery_mv:u16 | max_temp_c:u8 | legacy_fault_mask:u8`; `battery_mv=0` means sensing is not fitted and must not be interpreted as undervoltage. Additive telemetry `0x84` is 46 bytes, all multibyte fields big-endian: `fault_mask:u32 | mode:u8 | reset_cause:u8 | imu_age:u16 | wheel_l_age:u16 | wheel_r_age:u16 | servo_age[4]:u16 | imu_errors:u16 | wheel_errors[2]:u16 | servo_errors[4]:u16 | wheel_l_{timeout,checksum,protocol}_errors[3]:u16 | wheel_r_{timeout,checksum,protocol}_errors[3]:u16`. Age `0xffff` means no valid frame has ever been received. The per-device `*_errors` totals are saturated sums of timeout, checksum, and protocol failures; the trailing wheel breakdown splits that total into its three constituents for DDSM diagnostics (timeout = no reply, checksum = corrupt frame, protocol = UART hardware error). Reset-cause bits are pin, power-on, software, independent-watchdog, window-watchdog, low-power, and brownout in bits 0 through 6. Decoders may ignore unknown additive telemetry types.
