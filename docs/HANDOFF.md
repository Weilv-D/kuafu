...
按"忽视既有审查、仅从源码重推"的要求执行的全库审查（固件全部模块 + Pi5 运行时 + 跨层契约），共修复 3 项发现并全部带回归验证；有界残留与保留决策清单见 `docs/validation/stm32-firmware-2026-09-20-4.md`。对新接手者行为相关的：

1. **固件接收序列门支持对端重启重同步（中）**：与 Pi 侧解码器的 8 帧重同步完全镜像——连续 8 帧"CRC 合法、非 HELLO、仅被序列门拒绝"即判定 Pi 重启、重置基线，下一帧开启新会话。此前 Pi 重启时若其启动期唯一的 HELLO 帧在链路上损坏，新会话心跳会被单调序列门整体黑洞（时长可达上一会话全部序号数，50Hz 下以分钟计），ACTIVE 被锁死直到 STM32 断电。载荷校验失败的帧不计入（噪声打不开门）；任一帧被接受即清零计数，孤立重复帧仍被丢弃。计数器打包进既有 `have_rx_sequence` 状态字节，SWD 符号表不变。
2. **两个公共编码器对非有限输入取安全哨兵（低）**：`ddsm_build_torque`/`ddsm_build_speed` 对 NaN/Inf 编码为零（滑行），`servo_angle_to_tick` 对非有限角返回标定驻留 tick——三处原先都会到达 C 层未定义的 float→int 铸型（NaN 与任何钳位界比较均为假）。当前调用链上游已验证有限性故不可达；防护针对未来调用者，与 `quantize_i16`/`bmi088_read_temp` 同类。
3. **有界残留（已评审保留，不改码）**：极端中断延迟（>12ms）下迟到的 TX 完成中断可误完成 DDSM 新事务（损失一帧 + 一次健康误记，下个期限自愈）；ST3215 转矩使能帧协议无回复、"verified" 为软件真值（FAULT 周期重发 + 俯仰代理兜底，寄存器 40 读校验因无现场证据推迟）；`Error_Handler` 在 IWDG 下表现为复位循环（bring-up 致命态的既定自恢复姿态）。

主机测试全过（含 4 条新回归）；Keil 0 错 0 警告（RW/ZI 与上轮基线完全一致，无新增数据符号）；**SWD 符号地址对照新 map 23/23 不变**；Pi 侧 67 项 pytest + 平衡 trace 离线 5 项全过。

#### 2026-09-20 第三次审查（独立第五轮）

按"忽视既有审查、仅从源码重推"的要求执行：先独立推导架构与全部模块（含对照 HAL 驱动源码逐条核对中断/错误语义、逐字段核对 Pi 侧协议契约），之后才读历史记录核对不变量——此前四轮的修复全部复核为仍然成立。本轮共改进 6 项并全部验证；完整清单与证据见 `docs/validation/stm32-firmware-2026-09-20-3.md`。对新接手者行为相关的：

1. **舵机写帧的两条失败路径都走显式发送中止**：写帧（sync-write/torque）无应答，唯一完成事件是 TX 完成中断；丢失完成中断（20ms 截止期）与 UART 错误（`error_pending`）两条恢复路径现在都执行"AbortTransmit + 解析器复位"后回 IDLE。错误风暴正是丢完成中断的典型诱因，任一路径缺少中止都会把 HAL 发送状态永久卡在 `BUSY_TX`——此后所有腿写入失败、100ms 腿保持窗过期、监督器关闭轮门，且截止期兜底因 phase 已 IDLE 永不触发。
2. **ST3215 RX 环的重挂载保留圈真值**：环的绝对生产游标 = lap×size+写索引，lap 真值来自传输完成中断。重挂载（错误后 abort + 重启 DMA）后消费端锚定到当前圈边界（`dma_rx_ring_rebase`），已计圈与累计 overrun 诊断不丢——同步 abort 会完成旧流，竞态圈是真实的（与 Pi 侧 `pi_transport_reset` 同一语义）；启动时环先于 DMA 启动初始化。任一圈丢失或误抹都会让游标偏一圈（64B），把已覆写旧字节重新喂给解析器成虚假校验和失败。
3. **诊断/健康/FAULT 遥测在墙钟节拍发送**（250Hz 诊断 + 10Hz 健康，见 §7.5）：IMU 通路失效（I2C 死或 DRDY 停摆）时，250Hz 控制节照常锁存故障，诊断帧照常告诉 Pi 什么坏了、坏了多久；状态遥测（IMU/关节帧）有意留在 DRDY 守卫内——融合停摆后其数值冻结无意义，age 字段已承载该信息。
4. **平衡 trace 的首个采样带 INIT 启动标记**（`balance_trace_init` 自动置位）：dump 无需对照复位原因即可看出启动边界。
5. **遥测量化对非有限值取零哨兵**（`quantize_i16`）：NaN/Inf 不再到达 float→int 铸型（C 层未定义行为），协议零值本就是"不可用/安全"哨兵。
6. **`pi_link_on_tx_error` 与完成回调同防护**：无发送进行时（`tx_active==0`）错误回调不弹队列——它与我们的 TX 无关，弹掉 merely 排队的遥测帧纯属损失；停滞队列由下一次发送自愈。

主机测试全过（含四条新回归）；Keil 0 错误 0 警告；**SWD 符号地址对照新 map 全部不变**（新增状态都在栈或既有字段内）。
...
- **250Hz 控制节**在墙上时钟期限执行（与 DRDY 无关），一次期限跑完一条线性序列：安全状态机 → 监督器判决 → `g_wheel_output_gate` 门控 → LQR → trace，随后才是总线派发。门控判决持久到下个期限，LQR 与派发同守一门；**诊断/健康/FAULT 遥测同样按墙钟节拍（250Hz 诊断 + 10Hz 健康）发送**，DRDY 停摆或 IMU 通路失效只冻结状态遥测（IMU/关节帧——其数值本已冻结无意义），故障检测、取证与对 Pi 的故障播报不受影响。
