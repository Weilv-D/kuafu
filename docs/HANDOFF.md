...
启动状态机为 `WAIT_POWER → IMU_DISCOVERY → GYRO_CALIBRATION → ACTUATOR_DISCOVERY → READY`。各阶段在条件未满足时**持续重试**，不会因上电瞬间的扰动而锁死。陀螺标定需要机器人静止累计 1000 个静默样本（@250Hz ≈ 4 s），因此放地上后保持静止数秒即可完成启动并进入平衡。
