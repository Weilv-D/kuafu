...
| P5 checkpoint | resume test | model/optimizer/LR/entropy/curriculum/RNG/schema/hash restored |
...
|---|---|
| B0 | poles, signs, position hold, velocity/yaw tracking pass |
...
| S2 | D0 MAE at most 5 mm, roll at most 2 degrees, transition at most 0.5 s |
...
| S6 | 0.5–2 N s pushes recover within 2 s at least 90%, exceed baseline |
| S7 | mixed frozen holdout meets every applicable bucket minimum |

Each stage is evaluated only every 20–50 updates; two consecutive evaluations are required for advancement; one advancement at most per evaluation; 20–30% learned scenarios remain active. Frozen holdout seeds never control curriculum advancement.
