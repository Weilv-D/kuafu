...
# ============================================================
# 基层控制增益 — yaw 跟踪 + roll 调平 (STM32 1kHz 兜底层, sim 同源)
#   基层 = LQR pitch(已有 K) + yaw 差速跟踪 + roll 左右D0差PD
#   RL 残差叠在基层之上; RL 失效 → pitch/yaw/roll 三轴仍有兜底
#   增益为保守初值, 首件实测 + PPO 后标定
# ============================================================
# yaw: 原地 yaw 跟踪/大阻尼会耦合 pitch 不稳定 (同轴轮 IP 物理特性, verify 标定)。
#   故基层仅做条件阻尼 — |ωz| < YAW_DAMP_THRESH 时小阻尼抑制低频漂移; 大 ωz 关闭。
#   yaw 命令跟踪完全交 RL (非安全 DOF)。
YAW_KD = 0.05           # yaw 阻尼 (N·m·s²/rad), 仅低 ωz 时生效
YAW_DAMP_THRESH = 0.3   # rad/s, 仅 |ωz| < 此值时施加阻尼

# roll: ΔD0 = -Kp_r·roll - Kd_r·ωx → 左右 D0 差 (左+=Δ/2, 右-=Δ/2)
# roll 单位 rad, D0 单位 mm (env 内转 rad via HIP_STROKE)
ROLL_KP = 0.15          # roll 调平比例 (mm/rad → 经 HIP_STROKE 换算), 保守初值
ROLL_KD = 0.03          # roll 阻尼
