...
# ============================================================
# 载荷与整机质量（KUAFU §5.4 质量分布）
# ============================================================
M_TOT = 2.205           # 整机总质量 kg（含充电宝）
F_GRAV = M_TOT * G / 2.0   # 单腿静载 N（整机均分两腿）≈ 10.82
F_DES = 30.0            # 设计载荷 N/腿（含 3× 冲击系数）

# 腿部集中质量模型（KUAFU §6.3 拉格朗日动力学）
M_CRANK = 0.030         # 曲柄（大腿）等效质量 kg（质心 a/2）
M_LINK = 0.040          # 连杆（小腿）等效质量 kg（质心中点）
...
# ============================================================
MC = 0.70               # cart-pole cart 质量 kg（轮 + 髋组件）
MP = 1.51               # cart-pole pendulum 质量 kg（机身）
LP = 0.0711             # pendulum 质心高 m（驻留 COM，轮轴以上）
...
LQR_K = np.array([-4.47, -52.96, -5.52, -4.09])
OMEGA_N = 14.57         # 不稳定自然频率 rad/s（周期 0.43 s，摔倒 ~69 ms）
