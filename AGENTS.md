...
Training difficulty is an 8-axis independent curriculum (`rl/train/curriculum.py`): `command, d0, dr, latency, slope, step, rough, push`. Each axis advances/falls back on per-axis done-env episode statistics (`AXIS_CONFIG`). Terrain/perturbation axes use a pure survival gate; only `d0` keeps a tracking anti-cheat gate calibrated to the physical noise floor. The `command` axis is **pinned at max level** (fixed full-range velocity commands, legged_gym / MuJoCo Playground / RMA standard) and is never re-evaluated — velocity command tracking is a foundational skill learned from step one under the LQR baseline, not a curriculum-gated axis. Difficulty is sampled per env in a band `[0.8*level, level]` per axis. Rough terrain is a MuJoCo heightfield whose data is rewritten each reset (MJX supports `hfield×capsule`; wheels are capsules).
