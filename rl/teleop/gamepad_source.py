...
标定手柄轴/按钮映射: python -m rl.teleop.calibrate_native
(原生读 /dev/input/js0, 不依赖 pygame, 避免 BLE 设备事件丢失)
...
if __name__ == "__main__":
    # 标定工具已独立到 calibrate_native.py (直接读 /dev/input/js0,
    # 不依赖 pygame/SDL, 避免 BLE 设备事件丢失问题)
    print("标定手柄请运行: python -m rl.teleop.calibrate_native")
    print("本模块是运行时手柄源, 不是标定工具。")
