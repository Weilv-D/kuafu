 ...
 */
static int solve_crank(float hip_x, float d0, float branch_sign, float *out_angle) {
...
int kinematics_solve_ik(float D0, float *q_hip_A, float *q_hip_B) {
    ...
    /* Hip pivots: A chain (front) at -c, B chain (back) at +c, per sim model
    ...
    if (!ref_valid) {
        if (solve_crank(hip_x_A, KIN_MIN_LEG_D0, -1.0f, &ref_A) != 0 ||
            solve_crank(hip_x_B, KIN_MIN_LEG_D0, +1.0f, &ref_B) != 0) {
            return -1;
        }
        ref_valid = 1;
    }
    ...
    float abs_A, abs_B;
    if (solve_crank(hip_x_A, d0_clamped, -1.0f, &abs_A) != 0) return -1;
    if (solve_crank(hip_x_B, d0_clamped, +1.0f, &abs_B) != 0) return -1;
    ...
    *q_hip_A = -(abs_A - ref_A); /* front chain (rad), 0 at dwell */
    *q_hip_B = -(abs_B - ref_B); /* back chain  (rad), 0 at dwell */
