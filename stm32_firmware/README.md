...
2.  **主循环任务调度**：
    ...
    *   将 4 ms 的控制周期划分为 4 个分时时间槽：
        *   **Slot 0**：每周期计算一次 LQR 平衡控制（叠加 Pi 残差力矩、同步 Pi 速度/转向设定值），发送左轮指令并阻塞等待反馈，超时上限 2 ms；接收前清除 RS485 自回显。
        *   **Slot 1**：发送右轮指令（复用 Slot 0 缓存的力矩），阻塞等待反馈，超时上限 2 ms；接收前清除 RS485 自回显。
        *   **Slot 2**：通过 USART6 接口向树莓派发送姿态与关节状态数据包。
        *   **Slot 3**：发送诊断数据包并执行系统级安全状态机与阀限监测。
3.  **50 Hz 髋关节舵机控制**：
...
上位机与固件之间的髋关节舵机接口以本固件为单一真源，约定如下：

*   **角度参考系**：树莓派下发 **sim 帧关节角**（与 `rl/kuafu.xml` 一致），驻留态（dwell，D0=58mm）对应角度 **0**。
