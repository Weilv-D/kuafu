...
- `rl/env/contract.py`: versioned frames, units, signs, observation/action interface, and protocol ranges. Current schema: `v1.2.0` (`v1.1.0` policy artifacts are invalidated for ACTIVE deployment).
- `rl/env/kuafu_mjx_env.py`: 500 Hz physics, 250 Hz baseline controller, 50 Hz residual policy.
- `rl/train/`: reparameterized tanh-squashed PPO, 8-axis independent curriculum, atomic schema-aware checkpoints with CPU-first loading.
...
- `rl/teleop/`: gamepad/keyboard teleop with a two-state arm/disarm safety model, input shaping, and host tests.
- `pi5_runtime/`: ONNX actor loop, versioned UART codec, and the teleop command arbiter.
...
The policy is a tanh-squashed diagonal Gaussian with a reparameterized entropy and a numerically stable log-prob that accounts for the Jacobian term. PPO uses `init_noise_std = 0.4`, `gamma = 0.995`, and `num_steps_per_env = 96`. Curriculum operates over eight independent axes: `command, d0, dr, latency, slope, step, rough, push`. Friction domain randomization samples actual coefficients in `[0.3, 1.2]`. Push perturbation uses impulse-based sampling of 0-2 N·s with random timing. Rough terrain is a MuJoCo heightfield rewritten each reset. Roll leveling gains are `ROLL_KP = 190 mm/rad` and `ROLL_KD = 5.0`.
...
PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 rl/.venv/bin/python -m pytest rl/train/tests rl/teleop/tests -q
```

`PYTEST_DISABLE_PLUGIN_AUTOLOAD=1` avoids unrelated ROS plugins installed in the host Python environment. Firmware host C tests build with CMake under `stm32_firmware/tests/`.
